...
public typealias CameraImagesCompletion = ([UIImage]?, [PHAsset]?, Bool) -> Void

public typealias OpenCameraCompletion = () ->  Void
...
public extension CameraViewController {
    public class func imagePickerViewController(croppingEnabled: Bool, completion: CameraViewCompletion) -> UINavigationController {
        ...
        imagePicker.onSelectionComplete = { [weak imagePicker] asset in
            if let asset = asset {
                let confirmController = ConfirmViewController(asset: asset, allowsCropping: croppingEnabled, mode: .preview)
    ...
    // MARK: - 修改 (照片多选)
    public class func imagesPickerViewController(croppingEnabled: Bool, completion: CameraImagesCompletion) -> UINavigationController {

//        let imagePicker = PhotoLibraryViewController.photoManger()
        
        let imagePicker = PhotoLibraryViewController()
        let navigationController = UINavigationController(rootViewController: imagePicker)
//        navigationController.modalTransitionStyle = UIModalTransitionStyle.CrossDissolve
        imagePicker.onSelectionAssetsComplete = { [weak imagePicker] assets, photoGraph in

            images.removeAll()
            if let graph = photoGraph {
                images.append(graph)
                completion(images, assets, false)
                return
            }
            for asset in assets {
                ...
                PHImageManager.defaultManager().requestImageForAsset(asset, targetSize: PHImageManagerMaximumSize, contentMode: .AspectFill, options: PHImageRequestOptions()) { image, _ in
                    if let image = image {
                  
                        // 压缩图片
                        let newImage = image.imageWithScale(UIScreen.mainScreen().bounds.size.width)
        ...
        /*
        imagePicker.onOpenCameraComplete = { [weak imagePicker] in
    ...
    public class func imagesRemoveAll() {
    
        images.removeAll()
    }
    
    public class func removeSeletced(tag: Int) {
        
        images.removeAtIndex(tag)
    }
}
...
public class CameraViewController: UIViewController {
    ...
    static var group = dispatch_group_create()
    ...
    var imagesCompletion: CameraImagesCompletion?
    var openCamera: OpenCameraComplete?
    ...
    public init(croppingEnabled: Bool, allowsLibraryAccess: Bool = true, imagescompletion: CameraImagesCompletion) {
        
        super.init(nibName: nil, bundle: nil)
        imagesCompletion = imagescompletion
        allowCropping = croppingEnabled
        cameraOverlay.hidden = !allowCropping
        libraryButton.enabled = allowsLibraryAccess
        libraryButton.hidden = !allowsLibraryAccess
    ...
    private func startConfirmController(asset: PHAsset) {
        let confirmViewController = ConfirmViewController(asset: asset, allowsCropping: allowCropping, mode: .preview)
